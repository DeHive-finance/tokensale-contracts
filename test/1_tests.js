...
const PRE_SALE_START = 1616594400;
const PRE_SALE_END = 1616803140;

const PUBLIC_SALE_START = 1618408800;
const PUBLIC_SALE_END = 1618790340;
...
describe('Purchase DHV test coverage', () => {
    let deployer;
    let user1, user2, user3, treasury;
    ...
    before(async () => {
        [
            deployer, user1, user2, user3, treasury
       ...
       deHiveTokensale = await deployProxy(DeHiveTokensaleMock, 
        ...
        await testToken.mint('10000000000000000000000000', {from: deployer});
        await testToken.transfer(user2, '10000000000000000000000000', {from: deployer});
        await testToken.approve(deHiveTokensale.address, '10000000000000000000000000', {from: user2});

        await testToken.mint('10000000000000000000000000', {from: deployer});
        await testToken.transfer(user3, '10000000000000000000000000', {from: deployer});
        await testToken.approve(deHiveTokensale.address, '10000000000000000000000000', {from: user3});

        await deHiveTokensale.adminSetMaxDHV('10000000000000000000000000', {from: deployer});
    });
