...
describe('Gas estimate', () => {
  ...
  describe('Gas test', () => {
    ...
    it('Call purchaseDHVwithNUX() and get gas estimate', async() => {
      ...
      const gasEstimate = await deHiveTokensale.purchaseDHVwithERC20.estimateGas(
        testTokenAddress,
        1000000,
        { from: user1 }
      );
    ...
    it('Call adminWithdrawERC20 and adminWithdraw and get gas estimate',
      async() => {
        ...
        await timeMachine.advanceTimeAndBlock(PUBLIC_SALE_END - tmp_time + 86400);

        const gasEstimate = await deHiveTokensale.adminWithdrawERC20.estimateGas(
          testTokenAddress,
          { from: deployer }
        );
    ...
    it('Call adminSetVestingStart() and get gas estimate', async() => {
      const gasEstimate = await deHiveTokensale.adminSetVestingStart.estimateGas(
        VESTING_START,
        { from: deployer }
      );
